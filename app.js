...
const S = {
  ...
  period: 'year', cmp: null, dashVeh: '', cmpVeh: '', vehExpVeh: '', vehExpGran: 'month', bankCountries: null, gran: 'month', customBanks: [], theme: 'light', dstatType: '', histBadOnly: null
};
...
async function bumpVehicleKm(aracId, deltaKm) {
  ...
  await safeWrite(() => db.vehicles.update(v.id, {kmNow: yeni}));   // WT-12
}
function applyTheme() {
...
function applyI18n() {
  ...
  if (typeof scanOrphans === 'function') scanOrphans();   // WT-09/C
...
async function renderDashboard() {
  ...
  const wd = conv.filter(r => r.mesafeKm > 0);           // km metrikleri de aynı kümeden
...
async function renderStats() {
  ...
  const segs = [
    {name: 'DC', kwh: cur.filter(r => r.tip === 'DC' && r.firma !== home).reduce((s, r) => s + r.kwh, 0), col: '#16A34A'},
    {name: 'AC', kwh: cur.filter(r => r.tip !== 'DC' && r.firma !== home).reduce((s, r) => s + r.kwh, 0), col: '#1B5FAA'},
    {name: home, kwh: cur.filter(r => r.firma === home).reduce((s, r) => s + r.kwh, 0), col: '#7DC855'}
  ].filter(x => x.kwh > 0);
...
const SETTING_KEYS = ['country','currency','unit','lang','advOpen','defaultVehicleId',
  'onboarded','cmp','bankCountries','customBanks','gran','theme'];
