...
defaultHint:'★ varsayılan · 🛣️ kilometre · 📷 fotoğraf · × arşivle',
...
defaultHint:'★ default · 🛣️ odometer · 📷 photo · × archive',
...
defaultHint:'★ Standard · 🛣️ Kilometerstand · 📷 Foto · × Archiv',
...
defaultHint:'★ défaut · 🛣️ compteur · 📷 photo · × archive',
...
defaultHint:'★ predeterminado · 🛣️ kilometraje · 📷 foto · × archivo',
...
defaultHint:'★ predefinito · 🛣️ contachilometri · 📷 foto · × archivio',
...
async function renderDashboard() {
  ...
  const odoV = S.dashVeh ? vehicles.find(v => String(v.id) === S.dashVeh)
...
async function renderSettings() {
  ...
  $('set-vehicles').innerHTML = vehicles.length ? vehicles.map(v => {
    ...
    return `<li data-vid="${v.id}">
      ...
      <button class="cam" data-odo="${v.id}" title="km güncelle">🛣️</button>
  ...
  $('set-vehicles').querySelectorAll('[data-odo]').forEach(li =>
    li.addEventListener('click', async e => {
      ...
      const upd = {kmNow: km};
      if (v.kmStart == null) {
        const sIn = prompt(t('odoStartPrompt', {u: S.unit}), Math.round(distDisp(km)));
        const sVal = pf(sIn);
        upd.kmStart = (!isNaN(sVal) && sVal >= 0)
          ? Math.round(S.unit === 'mi' ? sVal * MI : sVal) : km;
      }
