...
const S = {
  ...
  period: 'year', cmp: null, dashVeh: '', cmpVeh: '', bankCountries: null, gran: 'month', customBanks: [], theme: 'light', dstatType: ''
};
...
document.querySelectorAll('[data-themetoggle]').forEach(b =>
  b.addEventListener('click', async () => {
    ...
    applyTheme();
    renderDashboard();   // grafik/donut renkleri temaya göre yeniden çizilsin
  }));
...
async function renderDashboard() {
  ...
  if (distKm >= 20) {
    $('d-100').textContent = money2(netD / distKm * 100 * f);
    $('d-100-g').textContent = money2(grossD / distKm * 100 * f);
  } else {
    ...
    if (oDist >= 20) {
      ...
      $('d-100').textContent = money2(aNet / oDist * 100 * f);
      $('d-100-g').textContent = money2(aGross / oDist * 100 * f);
    } else {
      $('d-100').textContent = '—';
      $('d-100-g').textContent = '—';
    }
...
$('btn-support').addEventListener('click', () => {
  window.open('https://github.com/sponsors/hasdemirfatih', '_blank', 'noopener');
});
