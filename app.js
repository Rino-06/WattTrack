...
jsonDone:'Backup JSON scaricato',csvDone:'CSV scaricato',noData:'Nessuna ricarica',numRange:'{f} deve essere tra {min} e {max}',socOrder:'La percentuale finale deve essere maggiore di quella iniziale',fldKwh:'Energia (kWh)',fldAmount:'Importo pagato',fldDisc:'Sconto',fldRate:'Tasso di cambio',fldDist:'Distanza percorsa',fldOdo:'Contachilometri',fldDurH:'Durata ricarica (ore)',fldDurM:'Durata ricarica (minuti)',fldSoc:'Percentuale di carica',fldUnitPrice:'Prezzo per kWh',badDataFound:'{n} registrazioni contengono valori non validi',showLbl:'Mostra',dismissLbl:'Chiudi',dateNeeded:'Inserisci una data valida',futureDate:'Stai inserendo una registrazione con data futura',sessions:'ricariche'}
};
...
// düzeltmeyi o yapsın.
const warnings = new Map();   // id -> {msg, actionLbl, action}
function setWarning(id, w) {
  if (w) warnings.set(id, w); else warnings.delete(id);
  renderWarnings();
}
function renderWarnings() {
  const host = $('d-warnings');
  if (!host) return;
  host.innerHTML = '';
