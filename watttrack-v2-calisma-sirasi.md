...
| **WT-58** | Veri girişinden girilen km'nin Aracım'daki toplam km'ye yansımaması | Aracım 1 | Ana sayfa `odoNowOf()` kullanıyor (kayıt ve elle giriş arasında büyük olanı alır, `calc.js:289`); Aracım sayfasında bazı yerlerde `v.kmNow` **doğrudan** okunuyor (`ui/vehicle.js:241`). Muhtemel neden bu asimetri. |
...
| **WT-62** | Ana sayfadaki boş "—" kutusu | Anasayfa 1 | **Kök neden bulundu.** `ui/shell.js:724-728`, `#page-dashboard .d-data` sınıfındaki **her** kutuyu görünür yapıyor; `#d-budget` (`index.html:553`) bütçe girilmediği için `butceCiz()` tarafından gizlenmişti, `syncEmptyStates()` onu geri açıyor ve varsayılan "—" görünüyor. `d-odo-wrap` için zaten istisna yazılmış (satır 726). **Çözüm ikinci bir istisna eklemek olmayacak** — bir sonraki kendi görünürlük mantığı olan kutu aynı tuzağa düşer. Önce `showScreen`'de `RENDER[name]?.()` beklenerek (`await`) render'ın görünürlüğün sahibi olması denenecek; bu, "sayfa değiştirince kayboluyor" yarışını da doğrudan çözer. |
